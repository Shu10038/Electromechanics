...
//    				+target[i2]*(a2-alpha[i2])*kernel(k,i2)+b_old-b;
  ...
  /******* main **************************/
    public void mainSMO(double traiSet[],int traiLabel[],int numData,int dimension){
      ...
      // wの更新
        for(int i = 0 ; i < nData ; ++i){
        	for(int j = 0 ; j < dim ; ++j){	
        		w[j] += alpha[i]*target[i]*point[i*dim+j];	
        	}		
        }
      ...
      // bの更新       	
        ...
        for(int i = 0 ; i < nData ; ++i){
        	bBuff = 0.0;
        	for(int j = 0 ; j < dim ; ++j){
        		bBuff += w[j] * point[i*dim+j];	
        	}
        	if(bBuff >= bMax) bMax = bBuff;
        	if(bBuff <= bMin) bMin = bBuff;
        }
        this.b = -1.0/2.0*(bMax + bMin);
 ...
 //==========　識別 ==========================================
  ...
  /******* ある点におけるSVM出力 *******************/
    double outPut(int atPoint){
    	double sum = 0.0;
    	for(int k = 0; k < nData ; ++k){
    		sum += alpha[k] *target[k]* kernelCF(k, atPoint);
    	}
    	return sum;
    }
